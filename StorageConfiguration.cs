...
internal static partial class Launcher
{
	...
	private static void WriteLauncherUpdateResult(string status, string message)
	{
		...
		{
			Console.WriteLine("[Storage] 런처 업데이트 결과 기록 실패: " + ex.Message);
		}
	}
	...
	private static string ConsumeLauncherUpdateResult()
	{
		...
		{
			try { File.Delete(path); } catch (Exception ex) { Console.WriteLine("[Storage] 런처 업데이트 결과 파일 삭제 실패: " + ex.Message); }
		}
	}
	...
	private sealed class DataLocationForm : Form
	{
		...
		public DataLocationForm(string executableDirectory, DataStorageSettings current, string initialError)
		{
			...
			Font = new Font(ThemeFonts.Body, 11F);
			...
			heading.Font = new Font(ThemeFonts.Display, 18F, FontStyle.Bold);
			...
			userOption = CreateLocationOption(korean ? "사용자 데이터 폴더 (권장)" : "User data folder (Recommended)", userPath);
			portableOption = CreateLocationOption(korean ? "현재 폴더 하위 (Portable / 고급)" : "Current folder (Portable / Advanced)", portablePath + (Directory.Exists(portablePath) ? (korean ? " · 기존 데이터 발견" : " · existing data found") : string.Empty));
			customOption = CreateLocationOption(korean ? "사용자 지정 폴더 (고급)" : "Custom folder (Advanced)", korean ? "직접 선택한 안전한 폴더를 사용합니다." : "Use a safe folder selected by you.");
