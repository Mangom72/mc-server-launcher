...
internal static partial class Launcher
{
	...
	private sealed class ContentManagerForm : Form
	{
		...
		public ContentManagerForm(LauncherOptions launcherOptions)
		{
			...
			searchBox.Font = new Font("Segoe UI Variable Text", 11F);
			...
			projectTitleLabel.Font = new Font("Segoe UI Variable Display Semib", 14F);
	...
	{
		Uri uri;
		if (!Uri.TryCreate(url, UriKind.Absolute, out uri) || uri.Scheme != Uri.UriSchemeHttps || !uri.Host.Equals("cdn.modrinth.com", StringComparison.OrdinalIgnoreCase))
		{
			throw new InvalidDataException("Modrinth 아이콘 주소를 검증하지 못했습니다.");
		}
		HttpWebRequest request = (HttpWebRequest)WebRequest.Create(uri);
		request.Method = "GET";
		request.UserAgent = GetLauncherIntegrationUserAgent();
		...
		using (HttpWebResponse response = (HttpWebResponse)request.GetResponse())
		{
			if (response.StatusCode != HttpStatusCode.OK || response.ResponseUri == null || !response.ResponseUri.Host.Equals("cdn.modrinth.com", StringComparison.OrdinalIgnoreCase) || response.ContentLength > 8388608L)
			{
				throw new WebException("Modrinth 아이콘 응답을 검증하지 못했습니다.");
			}
			using (Stream input = response.GetResponseStream())
	...
	private static string AnalyzeServerFailure(IEnumerable<string> lines)
	{
		StringBuilder builder = new StringBuilder();
