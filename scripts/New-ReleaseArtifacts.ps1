...
$metadata = [ordered]@{
    ...
    # v1.5.20 이하 런처의 허용 경로와 자산 이름을 유지합니다. GitHub가 이 별칭을 정식 저장소로 리디렉션합니다.
...
- 실행 파일에는 릴리스마다 폐기하는 자체서명 무결성 서명이 포함되지만 공개 신뢰 서명은 아니므로 Windows SmartScreen 경고가 표시될 수 있습니다.
