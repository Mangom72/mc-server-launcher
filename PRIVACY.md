...
The launcher accesses GitHub Releases for launcher updates; PaperMC, Purpur, Mojang, Fabric, Forge, and NeoForge services for server metadata and files; Eclipse Adoptium and its GitHub release assets for Java; Modrinth for content search, icons, and downloads; wsrv.nl to convert public Modrinth SVG/WebP icon URLs to PNG images capped at 256×256; and portchecker.io for public-IP and external-port checks initiated by the launcher. UPnP discovery happens only after an external connection check fails. The launcher does not read or transmit router administrator passwords.
