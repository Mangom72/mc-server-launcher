...
internal static partial class Launcher
{
	...
	private sealed class MultiServerDashboardForm : Form
	{
		...
		public MultiServerDashboardForm(string rootDirectory, bool mainServerIsBusy)
		{
			...
			profileActions.WrapContents = true;
		...
		private void StopSelected()
		{
			...
			{
				if (ShowMineHarborDialog(this, ManagedText(
					"서버를 강제 종료하면 저장되지 않은 월드 데이터가 손상될 수 있습니다.\n\n정말로 강제 종료하시겠습니까?", 
					"Force stopping the server may corrupt unsaved world data.\n\nAre you sure you want to force stop?"), Text, MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
				{
					session.Status = ManagedText("강제 종료 중", "Force stopping");
					RenderProfiles();
		...
		private void OnDashboardClosing(object sender, FormClosingEventArgs eventArgs)
		{
			...
			foreach (ManagedServerSession session in sessions.Values)
			{
				...
				{
					session.StopRequested = true;
					ManagedServerSession targetSession = session;
					ThreadPool.QueueUserWorkItem(state =>
					{
						try
						{
							targetSession.Process.StandardInput.WriteLine("stop");
							targetSession.Process.StandardInput.Flush();
						}
						catch (Exception ex)
						{
							Console.WriteLine("[Managed] 서버 닫기 명령 전송 실패: " + ex.Message);
						}
					});
				}
			}
	...
	private static void ParseManagedServerLine(ManagedServerSession session, string line)
	{
		if (line.StartsWith("[Launcher] child-pid="))
		{
			...
			int pid;
			long start;
			if (parts.Length == 2 && int.TryParse(parts[0], out pid) && long.TryParse(parts[1], out start))
			{
				session.ChildPid = pid;
				session.ChildStartTime = start;
			}
		}
