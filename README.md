...
| 방식 | 이런 경우에 추천합니다 | 받기 |
| --- | --- | --- |
| **Portable EXE** | 설치 없이 파일 하나로 바로 실행 | **[고정 링크로 다운로드](https://github.com/Mangom72/mc-server-launcher/releases/latest/download/MineHarbor.exe)** |
...
현재 소스 버전은 `v1.0.0`, 내부 빌드는 `26.2.45.31`입니다. MineHarbor 이름으로 배포된 Portable EXE는 같은 링크에서 계속 최신 파일을 받을 수 있습니다. 기존 설치의 `%LOCALAPPDATA%\MinecraftServerLauncher` 데이터는 자동으로 찾아 그대로 사용하며, 새 사용자 데이터 경로는 `%LOCALAPPDATA%\MineHarbor`입니다.
...
1. **[Portable EXE를 내려받아](https://github.com/Mangom72/mc-server-launcher/releases/latest/download/MineHarbor.exe)** 실행합니다.
...
- 삭제한 서버를 30일간 보관하는 휴지통과 복구·영구 삭제 관리
...
| --- | --- | --- |
| **Portable EXE** | Run one file without installation | **[Download from the permanent URL](https://github.com/Mangom72/mc-server-launcher/releases/latest/download/MineHarbor.exe)** |
...
Current source version: `v1.0.0` · internal build: `26.2.45.31`. MineHarbor releases keep the same permanent Portable URL. Existing data under `%LOCALAPPDATA%\MinecraftServerLauncher` is detected and preserved; new user-data installations use `%LOCALAPPDATA%\MineHarbor`.
...
1. **[Download the Portable EXE](https://github.com/Mangom72/mc-server-launcher/releases/latest/download/MineHarbor.exe)** and run it.
