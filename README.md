...
6. `서버 시작하기`를 누르고 표시되는 주소를 친구에게 전달합니다.
...
6. Press `Start server` and share the displayed address.
...
Installer builds use Inno Setup 6.7 or newer. The tag/manual GitHub Actions workflow creates the Portable EXE/ZIP, Windows installer, `SHA256SUMS.txt`, and `update.json`. Code signing is optional and runs only when certificate secrets are configured. See [CONTRIBUTING.md](CONTRIBUTING.md).
