<div align="center">
  <img src="launcher-icon.ico" width="96" height="96" alt="잔디 블록 모양의 Minecraft Server Launcher 아이콘">
  ...
  <p>
    <a href="https://github.com/Mangom72/mc-server-launcher/releases/latest"><img src="https://img.shields.io/github/v/release/Mangom72/mc-server-launcher?display_name=tag&amp;sort=semver&amp;style=flat-square&amp;color=3182F6" alt="최신 GitHub Release 버전"></a>
    <a href="https://github.com/Mangom72/mc-server-launcher/actions/workflows/build-release.yml"><img src="https://github.com/Mangom72/mc-server-launcher/actions/workflows/build-release.yml/badge.svg" alt="빌드 및 릴리스 워크플로 상태"></a>
    <a href="LICENSE"><img src="https://img.shields.io/github/license/Mangom72/mc-server-launcher?style=flat-square&amp;color=20C997" alt="MIT 라이선스"></a>
...
| 방식 | 이런 경우에 추천합니다 | 받기 |
| --- | --- | --- |
| **Portable EXE** | 설치 없이 파일 하나로 바로 실행 | **[고정 링크로 다운로드](https://github.com/Mangom72/mc-server-launcher/releases/latest/download/Minecraft-Server-Launcher.exe)** |
| **Windows 설치 프로그램** | 시작 메뉴, 선택적 바탕화면 바로가기, 제거 기능 사용 | **[최신 Release 열기](https://github.com/Mangom72/mc-server-launcher/releases/latest)** |
| **Portable ZIP** | README와 라이선스를 포함한 묶음 보관 | **[최신 Release 열기](https://github.com/Mangom72/mc-server-launcher/releases/latest)** |

현재 릴리스는 `v0.4.2`, 내부 빌드는 `26.2.45.30`입니다. Portable EXE는 항상 같은 링크에서 최신 파일을 받을 수 있습니다. 설치 프로그램으로 업데이트해도 `%LOCALAPPDATA%\MinecraftServerLauncher`의 서버 데이터는 유지됩니다.
...
| **한곳에서 운영**<br>여러 서버, 백업, 콘텐츠, 플레이어와 콘솔을 함께 관리합니다. | **외부 접속 지원**<br>기존 포트포워딩을 먼저 확인하고 필요한 경우에만 UPnP를 시도합니다. |

Paper, Vanilla, Purpur부터 모드 서버까지 지원하면서도 자주 쓰는 기능만 먼저 보여 줍니다. 상세 설정과 진단 도구는 필요할 때 열 수 있고, 한국어·영어 및 다크·라이트 모드를 제공합니다.

### 3단계로 시작하기

1. **[Portable EXE를 내려받아](https://github.com/Mangom72/mc-server-launcher/releases/latest/download/Minecraft-Server-Launcher.exe)** 실행합니다.
...
Minecraft 및 서버 종류에 맞춰 Java 8·11·16·17·21·25 중 필요한 런타임을 선택합니다. 호환 Java가 없으면 Eclipse Adoptium에서 받아 서버별 요구 버전에 맞춰 실행합니다. 직접 JAR은 제작자가 요구하는 Java 주 버전을 사용자가 지정할 수 있습니다.
...
- 화이트리스트, OP·DEOP, 추방, 차단·해제 플레이어 관리
...
flowchart TD
    ...
    B -- 예 --> D{현재 설정으로 외부 접속이 되나요?}
    D -- 예 --> E[기존 포트포워딩 유지 · 접속 주소 표시]
...
- 기존 매핑이 정상이면 UPnP를 실행하거나 설정을 변경하지 않습니다.
...
| 데이터 위치 | 경로 |
| --- | --- |
| 사용자 데이터 | `%LOCALAPPDATA%\MinecraftServerLauncher` |
| Portable 데이터 | EXE 옆의 `Minecraft-Servers-Data` |
| 사용자 지정 | 쓰기 가능하고 시스템 폴더가 아닌 선택 경로 |

기존 `Minecraft-Servers-Data`를 찾으면 우선 제안하지만 자동으로 이동·삭제·덮어쓰지 않습니다. 각 서버는 `servers\<프로필 이름>` 아래에서 월드와 설정을 분리해 보관합니다.
...
문제가 계속되면 민감 정보를 가린 `진단 묶음`을 만든 뒤 [GitHub Issues](https://github.com/Mangom72/mc-server-launcher/issues)에 증상과 재현 순서를 남겨 주세요. 보안 문제는 공개 이슈 대신 [보안 정책](SECURITY.md)의 방법으로 알려 주세요.

## 개발과 기여

Windows 10/11 x64, PowerShell 5.1 이상과 .NET Framework 4.x C# 컴파일러가 필요합니다.
...
| --- | --- | --- |
| **Portable EXE** | Run one file without installation | **[Download from the permanent URL](https://github.com/Mangom72/mc-server-launcher/releases/latest/download/Minecraft-Server-Launcher.exe)** |
| **Windows installer** | Start Menu, optional desktop shortcut, and uninstall support | **[Open the latest release](https://github.com/Mangom72/mc-server-launcher/releases/latest)** |
| **Portable ZIP** | Keep the launcher, README, and license together | **[Open the latest release](https://github.com/Mangom72/mc-server-launcher/releases/latest)** |

Current release: `v0.4.2` · internal build: `26.2.45.30`. The Portable EXE URL stays the same across releases. Installer upgrades preserve server data under `%LOCALAPPDATA%\MinecraftServerLauncher`.

### Minecraft servers without the setup maze

Minecraft Server Launcher is a Windows desktop app for creating and operating Paper, Vanilla, Purpur, Fabric, Forge, NeoForge, and custom-JAR servers. It prepares a compatible Java runtime, downloads and verifies server files, keeps profiles isolated, manages backups and content, and helps make a server reachable from outside your network.
...
| **Operate in one place**<br>Manage multiple servers, backups, content, players, and consoles. | **Connect safely**<br>Existing forwarding is checked first; UPnP is attempted only after a confirmed failure. |

### Quick start

1. **[Download the Portable EXE](https://github.com/Mangom72/mc-server-launcher/releases/latest/download/Minecraft-Server-Launcher.exe)** and run it.
...
- **Server profiles:** create, clone, import, rename, archive, select, run, and safely stop isolated servers; separate ports can run concurrently.
...
- **Updates and backups:** optional server auto-update, manual upgrades, staged profile restore, SHA-256 verification, retention, export, and rollback-aware launcher updates.
- **Content:** popular projects on entry, rich project details, and verified Modrinth plugin, mod, and data-pack installation.
- **Operations:** whitelist, OP, kick and ban controls; searchable console with separate warning, compatibility, and error filters.
...
- **Interface:** Korean/English, dark/light themes, concise tooltips, responsive windows, background loading, and visible progress.
...
The launcher checks the local TCP listener and current external reachability before touching UPnP. Working router settings are left unchanged. After a confirmed failure, it discovers UPnP, detects collisions, creates only the required TCP mapping and optional Query UDP mapping, then tests again.
...
Choose user data (`%LOCALAPPDATA%\MinecraftServerLauncher`), Portable data (`Minecraft-Servers-Data` beside the EXE), or a writable custom folder. Existing data is offered but never automatically moved, deleted, or overwritten.
...
- [Latest release](https://github.com/Mangom72/mc-server-launcher/releases/latest)
