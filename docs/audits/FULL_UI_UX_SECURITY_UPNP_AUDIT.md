...
## v1.7.0 후속 조치 (2026-07-20)

- 일반 외부 TCP 검사로는 응답 서비스가 현재 Minecraft 서버인지 식별할 수 없으므로, 열린 포트를 더 이상 `기존 포트포워딩 정상`으로 확정하지 않습니다. 후보 주소와 `서버 일치 미확인`을 표시하며 공유기 설정은 유지합니다.
- MineHarbor가 생성한 TCP 및 필요한 UDP 매핑이 완료되고 사후 외부 검사가 성공한 경우에만 `UPnP 매핑 확인됨`으로 분류합니다. TCP만 응답하거나 UDP가 불완전하면 부분·미확인 상태를 유지합니다.
- 포트 검사 HTTP 요청에 `NoCacheNoStore`와 `Cache-Control: no-cache, no-store`를 적용했습니다.
- 회귀 테스트는 일반 TCP 응답의 확정 오판 방지, 검증된 기본·대체 UPnP 주소, 반복 매핑 생성·정리와 소유권 보호를 포함하며 실제 공유기 설정은 변경하지 않습니다.
- UPNP-004와 UPNP-005의 상태 혼동 항목은 위 변경으로 해결했습니다. 제조사별 공유기, 이중 NAT/CGNAT 및 실제 인터넷 경로의 Minecraft 프로토콜 식별은 여전히 격리 장치 검증이 필요합니다.
...
이 감사 이후 수정 사항은 정식 패치 릴리스 `1.5.20`에 반영했다. P0~P3 항목 가운데 삭제 전 현재 매핑 소유권 재검증, 실행별 COM 매핑 기록과 비정상 종료 복구, TCP/UDP 부분 성공 표시, 외부 검사 3상태와 중복 실행 차단, 보수적 방화벽·CGNAT 판정, 비동기 정리 UI를 구현했다.
...
## 18. 전제품 후속 감사 및 수정 결과 (2026-07-20)

`1.5.21`~`1.5.22`에서 직접 SSDP/SOAP 기본 경로, Windows NATUPnP COM 백업, 최대 8개 대체 외부 포트, 실행 세대와 취소, 추가 응답 유실 복구 및 정확한 소유권 정리를 구현했다. 루프백 가짜 공유기로 TCP·UDP 매핑을 8회, 가짜 COM으로 12회 반복 생성·정리하고 기본 포트 충돌, 부분 실패, 요청 중 취소, 새 실행과 이전 지연 정리의 경합을 검증했다. 이번 감사에서도 같은 회귀가 통과했으며 실제 공유기 설정이나 외부 포트는 변경하지 않았다.

자동 업데이트와 선택형 명령 브리지 다운로드에서 GitHub 릴리스 별칭이 정식 저장소로 이동한 뒤 Release CDN까지 이어지는 리디렉션을 기존 허용 목록이 처리하지 못하는 결함을 확인했다. 메타데이터와 바이너리를 모두 수동 리디렉션으로 처리하고, 각 단계의 HTTPS·허용 호스트·사용자 정보 부재·리디렉션 수·응답 크기·최종 파일 크기와 SHA-256을 검증한다. 브리지 URL은 저장소, 릴리스 버전과 정확한 버전별 파일명까지 일치해야 한다. 공개 `v1.5.22` 브리지 자산을 이 경로로 실제 내려받아 크기와 SHA-256 일치를 확인했다.

Modrinth API·CDN·아이콘 변환 응답에도 리디렉션과 최대 크기를 적용했다. `wsrv.nl` 변환 요청의 원본 URL을 인코딩하고 출력 크기를 제한하며, 디코딩한 이미지가 가로·세로 4096픽셀 또는 총 16,777,216픽셀을 넘으면 거부한다. 개인정보 안내에는 아이콘 변환 서비스와 전송 항목을 한국어·영어로 추가했다.

실제 Windows 격리 프로필에서 최초 설정, 메인, 서버 관리, 백업, 콘텐츠, 빠른 명령·브리지 관리와 편집기, 한국어·영어, 다크·라이트 테마를 시각 확인했다. 실제 사용자 데이터 경로와 단일 실행 뮤텍스를 테스트 전용 비공개 재지정 지점으로 분리했고, 실제 서버 시작·명령 전송·플러그인 설치·UPnP 매핑은 수행하지 않았다. 영어 업데이트 노트 선택, 보조 창 DPI 배율과 버튼 줄바꿈, 타이머 해제, 스크린 리더 이름과 영어 문구를 보완했다.

GitHub Actions는 변경 가능한 태그 대신 전체 커밋 SHA를 사용하고 `persist-credentials: false`를 적용한다. 코드 서명 인증서 비밀은 확인·서명 단계에만 노출하고 임시 PFX는 성공·실패와 무관하게 정리한다. 이 조건은 `test.ps1` 정적 회귀 검사에 포함했다. 버전 갱신 도구는 저장소 기준 경로, 상호 배타 모드, 형식 검증과 UTF-8 무 BOM 저장을 사용한다.

최종 출시 전 기준 검증은 `PASSED=22`, `PORTABLE_VERSION_OK`, `PORTABLE_SMOKE_OK`, `BRIDGE_PROTOCOL_PASSED=8`, `MODERN_DIALOG_SCAN_OK`, `SECURITY_REGRESSION_SCAN_OK`이다. 실제 제조사별 공유기, NAT-PMP/PCP 전용 장치, COM 무응답, 125%·150%·200% DPI와 Narrator 장치 행렬은 여전히 사용자 승인을 받은 격리 환경에서만 수행해야 한다.
