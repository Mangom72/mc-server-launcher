...
- **Branch**: `codex/modern-ui-autocomplete-port-status-v1.7.0`
- **Status**: 현대형 공통 컨트롤·자동완성·보수적 외부 포트 상태·휴지통/창 닫기 UX 구현 및 로컬 릴리스 후보 검증 완료, PR·릴리스 진행 중
...
- 릴리스 게시 후 이 항목에 PR, 병합 커밋, Actions 실행, 공개 자산 및 이전 버전 자동 업데이트 검증 결과를 추가해야 합니다.
